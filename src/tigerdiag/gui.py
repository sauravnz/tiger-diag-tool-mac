...
from .service_interval import (
    SERVICE_DAYS_DEFAULT,
    SERVICE_DAYS_OPTIONS,
    SERVICE_DISTANCE_MAX_KM,
    SERVICE_DISTANCE_MIN_KM,
    SERVICE_DISTANCE_STEP_KM,
    format_service_interval,
    read_service_interval,
    reset_service_date,
    reset_service_distance,
)
...
<body>
      ...
      <div>
        ...
        <section id="service" class="panel">
          ...
          <div class="subpanel">
            ...
            <div class="row">
              <label>Distance
                <select id="distanceKm"></select>
              </label>
              <label>Time
                <select id="daysToService"></select>
              </label>
            </div>
            ...
            <div class="safety">
              ...
              <div class="row">
                ...
                <button id="resetDate" class="danger">Reset Date</button>
              </div>
            </div>
          </div>
        </section>
  ...
  <script>
    ...
    const distanceMinKm = __DISTANCE_MIN_KM__;
    const distanceMaxKm = __DISTANCE_MAX_KM__;
    const distanceStepKm = __DISTANCE_STEP_KM__;
    const serviceDaysOptions = __SERVICE_DAYS_OPTIONS__;
    const serviceDaysDefault = __SERVICE_DAYS_DEFAULT__;
    let snapshot = null;
    ...
    function formatDate(due) {
      return `${String(due.getDate()).padStart(2, "0")}/${String(due.getMonth() + 1).padStart(2, "0")}/${String(due.getFullYear()).slice(-2)}`;
    }

    function dateResetCommand(days) {
      const due = dueDateForDays(days);
      const wireDays = days + 1;
      return `5C ${hexByte(due.getFullYear() % 100)} ${hexByte(due.getMonth() + 1)} ${hexByte(due.getDate())} ${hexByte((wireDays >> 8) & 0xFF)} ${hexByte(wireDays & 0xFF)}`;
    }

    function distanceResetCommand(distanceKm) {
      return `33 ${hexByte(distanceKm / 100)}`;
    }

    function setupResetControls() {
      const distanceSelect = $("distanceKm");
      for (let km = distanceMinKm; km <= distanceMaxKm; km += distanceStepKm) {
        const option = document.createElement("option");
        option.value = km;
        option.textContent = `${km} km`;
        if (km === distanceMaxKm) option.selected = true;
        distanceSelect.appendChild(option);
      }

      const daysSelect = $("daysToService");
      serviceDaysOptions.forEach((days) => {
        const option = document.createElement("option");
        option.value = days;
        option.textContent = `${days} days`;
        if (days === serviceDaysDefault) option.selected = true;
        daysSelect.appendChild(option);
      });

      distanceSelect.onchange = updateResetPreview;
      daysSelect.onchange = updateResetPreview;
      updateResetPreview();
    }

    function updateResetPreview() {
      const distanceKm = Number($("distanceKm").value);
      const days = Number($("daysToService").value);
      const due = dueDateForDays(days);
      $("resetPreview").textContent =
        `Distance command: ${distanceResetCommand(distanceKm)}\\n` +
        `Date command: ${dateResetCommand(days)}\\n` +
        `Bike due date: ${formatDate(due)} (${days} days)`;
    }
    ...
    $("resetDistance").onclick = async () => {
      const distanceKm = Number($("distanceKm").value);
      if (!confirm(`Reset service distance to ${distanceKm} km?\\n\\nCommand: ${distanceResetCommand(distanceKm)}`)) return;
      await api("/api/reset", { kind: "distance", distanceKm, ...safety() });
    };
    $("resetDate").onclick = async () => {
      const daysToService = Number($("daysToService").value);
      const due = dueDateForDays(daysToService);
      if (!confirm(`Reset service date to ${formatDate(due)}?\\n\\nDays: ${daysToService}\\nCommand: ${dateResetCommand(daysToService)}`)) return;
      await api("/api/reset", { kind: "date", daysToService, ...safety() });
    };
...
class TigerDiagState:
    ...
    def reset_with_fresh_connection(
        self,
        kind: str,
        log,
        ...
        try:
            ...
            if kind == "distance":
                if distance_km is None:
                    raise RuntimeError("Distance is required.")
                return reset_service_distance(reset_conn, distance_km, log=log)
            if kind == "date":
                if days_to_service is None:
                    raise RuntimeError("Days to service is required.")
                return reset_service_date(reset_conn, days_to_service, log=log)
...
def create_handler(state: TigerDiagState):
    class Handler(BaseHTTPRequestHandler):
            ...
            try:
                ...
                with state.lock:
                    if self.path == "/api/reset":
                        ...
                        days_to_service = payload.get("daysToService")
                        ok = state.reset_with_fresh_connection(
                            kind,
                            log.append,
                            distance_km=int(distance_km) if distance_km is not None else None,
                            days_to_service=int(days_to_service) if days_to_service is not None else None,
                        )
        ...
        def _send_html(self):
            html = (
                HTML_TEMPLATE
                .replace("__DISTANCE_MIN_KM__", str(SERVICE_DISTANCE_MIN_KM))
                .replace("__DISTANCE_MAX_KM__", str(SERVICE_DISTANCE_MAX_KM))
                .replace("__DISTANCE_STEP_KM__", str(SERVICE_DISTANCE_STEP_KM))
                .replace("__SERVICE_DAYS_OPTIONS__", json.dumps(list(SERVICE_DAYS_OPTIONS)))
                .replace("__SERVICE_DAYS_DEFAULT__", str(SERVICE_DAYS_DEFAULT))
            )
